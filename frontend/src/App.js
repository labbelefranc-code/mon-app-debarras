...
const getABCDCategoryArticles = (abcdCategoryId, subcategoryName = null, subSubcategoryName = null) => {
  ...
  if (abcdCategoryId === 'C' && subcategoryName) {
    // Pour "Électronique/multimédia" et "Électrique", accès direct aux articles
...
const ModernAdminMultimediaElectriqueePage = ({ onGoBack, createArticle, updateArticle, deleteArticle, adminAuth, setCurrentStep, setIsAdminMode, loadAdminData }) => {
  ...
  const renderMultimediaElectriqueStructure = () => {
    if (!selectedMainCategory) {
      ...
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(ELEC_ADMIN_STRUCTURE.categories).map(([key, category]) => (
            <Card
              key={key}
              className="cursor-pointer hover:shadow-lg transition-all duration-200 hover:scale-105"
              onClick={() => setSelectedMainCategory(key)}
...
function App() {
  ...
  const addCustomItem = React.useCallback((description) => {
    setCustomItems(prev => [...prev, {
      ...
      estimated_price: 100.0 // Prix temporaire pour permettre les tests
    }]);
  }, []);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            {/* Note importante pour Vaisselle, vêtements et objets divers - UNIQUEMENT en haut de la page articles */}
            {selectedABCDCategory?.id === 'D' && selectedABCDSubSubcategory?.includes('Vaisselle') && (
...
const PhotoQuotePage = ({ onGoHome, onPhotosValidated }) => {
  ...
  const ABCDObjectsPage = () => {
                        ...
                        return (
                          <Card key={subcategoryName} 
                                ...
                                onClick={() => {
                                  ...
                                  // Pour les catégories AUTRES à accès direct, aller directement aux articles
                                  if (subcategoryName === 'Décoration') {
                                    setSelectedABCDSubSubcategory('Articles décoration');
                                  }
                                  if (subcategoryName === 'Accessoires') {
                                    setSelectedABCDSubSubcategory('Articles accessoires');
                                  }
                                  if (subcategoryName === 'Instruments de musique') {
                                    setSelectedABCDSubSubcategory('Articles instruments de musique');
                                  }
    ...
    // EXCEPTION : Pour JARDIN "Jardin et extérieur", ne PAS afficher cette page intermédiaire
    if (selectedABCDCategory && selectedABCDSubcategory && !selectedABCDSubSubcategory) {
      
      // EXCEPTION SPÉCIALE : JARDIN "Jardin et extérieur" va directement aux articles
      if (selectedABCDCategory.id === 'B' && selectedABCDSubcategory === 'Jardin et extérieur') {
        // Forcer la définition automatique pour aller aux articles
        setSelectedABCDSubSubcategory('Articles jardin');
        return null; // Le rendu va changer immédiatement
      }
      return (
        <div className="min-h-screen bg-gray-50">
          <div className="container mx-auto px-4 py-8">
